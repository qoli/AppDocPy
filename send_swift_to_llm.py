...
def generate_documentation(swift_code: str, output_file_path: str, component_type: str, file: str) -> None:
    """生成文檔"""
    request_data = {
        "model": MODEL_NAME,
        "prompt": f"{DOC_PROMPT}\n\n{swift_code}",
...
def process_file(file: str, file_path: str, index: int, total: int) -> None:
    """處理單個文件"""
    print(f"\n{'='*60}")
    ...
    try:
        ...
        # 步驟 2: 生成文檔
        generate_documentation(swift_code, output_file_path, component_type, file)
...
def main():
    ...
    for index, (file, file_path) in enumerate(swift_files, 1):
        process_file(file, file_path, index, total_files)
